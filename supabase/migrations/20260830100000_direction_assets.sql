...
begin
  ...
  -- Same palette already has a settled outcome: nothing to do, no billing.
  ...
  -- Actively claimed and still inside the reclaim window: someone else is
  -- working it, whatever palette they're working it for.
  --
  -- ⚠ clock_timestamp(), NOT now(). now()/transaction_timestamp() is fixed
  -- for the whole transaction — harmless for an ordinary updated_at, but
  -- this comparison and the token below need the real wall-clock instant so
  -- two claims made moments apart (even inside one transaction, as this
  -- migration's own test does) get genuinely distinct tokens and an
  -- accurate reclaim-window check.
  ...
  if v_row.status = 'claimed' then
    -- Stale claim: the invocation that held it is presumed dead. Retake the
    -- SAME reservation — do not touch direction_asset_daily_spend — so one
    -- image can never eat the daily budget twice.
  ...
  -- Pending, or ready/failed for a DIFFERENT palette (a regeneration): this
  -- is a genuinely new job and needs a fresh reservation against the cap.
