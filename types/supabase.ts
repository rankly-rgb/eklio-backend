...
//
// ⚠ HOW THIS PARTICULAR COPY WAS PRODUCED, because it is NOT the documented
// command and the difference is worth knowing.
//
// The documented command is:
//
//   supabase gen types typescript --db-url "$DB_URL" > types/supabase.ts
//
// run against a CLEAN REPLAY of supabase/migrations plus supabase/seed.sql,
// which is this repository's source of truth. That could not be run from the
// Content chantier's session 3 environment: there is no SUPABASE_ACCESS_TOKEN,
// Docker is not usable so `--local` has no stack to talk to, and
// api.supabase.com is refused by the network (curl returns 000).
//
// This copy was generated by Supabase's OWN generator through the Supabase MCP
// server, which does reach the project — the same endpoint the CLI's
// `--project-id` path calls. So these types are GENERATED, not hand-written;
// the previous copy had been hand-edited and had drifted so far that it was
// missing `content_items` entirely, four days after that table shipped.
//
// THE DIFFERENCE THAT REMAINS: this reflects the LIVE project, not a clean
// replay of the migration set. Those agree only as long as every change to the
